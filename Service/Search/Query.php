...
{
    public const DEFAULT_ROWS = 30;
    public const DEFAULT_START = 0;
    public const DEFAULT_FIELDS = '*,score';
    public const DEFAULT_QUERY = '*:*';
    public const DEFAULT_FACET = false;
    public const DEFAULT_STATS = false;
    public const DEFAULT_EDISMAX = false;
    public const DEFAULT_MLT = false;
    public const DEFAULT_WS = 'json';
    ...
    /**
     * Fields to set mincount for facet data.
     * @var array
     */
    protected $facet_mincount = [];

    /**
     * Fields to apply sort for facet data.
     * @var array
     */
    protected $facet_sort = [];
     ...
     * @return Query
     */
    public function setFQ($fq = null)
    {
        if ($fq) {
            $this->setOption('fq', $fq);
        }
     ...
     * @return Query
     */
    public function setFL($fl = null)
    {
        if ($fl) {
            $this->setOption('fl', $fl);
        }
     ...
     * @return $this
     */
    public function removeSort(string $key)
    {
     ...
     * @return Query
     */
    public function setEdismaxBoostQuery(string $query, ?array $bindValues, float $boost)
    {
        if ($bindValues !== null) {
            $query = $this->assembleQueryWithValues($query, $bindValues);
        }
    ...
    public function createRequest(Request $request): Request
    {
        ...
        if (count($this->facet_mincount)) {
            foreach ($this->facet_mincount as $facetName => $facetCount) {
                $request->addParam('f.'.$facetName.'.facet.mincount', $facetCount);
            }
        }

        if (count($this->facet_sort)) {
            foreach ($this->facet_sort as $facetName => $facetSort) {
                $request->addParam('f.'.$facetName.'.facet.sort', $facetSort);
            }
        }
        ...
        if (! empty($options['fq'])) {
            $request->addParam('fq', $options['fq']);
        }

        if (! empty($options['fl'])) {
            $request->removeParam('fl');
            $request->addParam('fl', $options['fl']);
        }
    ...
    /**
     * Specify mincount for a particular facet field.
     * @param string $facetName
     * @param int $count
     * @return $this
     */
    public function setFacetMincount(string $facetName, int $count)
    {
        $this->facet_mincount[$facetName] = $count;

        return $this;
    }

    /**
     * Specify sort for a particular facet field.
     * @param string $facetName, string $sort
     * @return Query
     */
    public function setFacetSort(string $facetName, string $sort)
    {
        $this->facet_sort[$facetName] = $sort;
